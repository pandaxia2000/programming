...
$outputexcelfilename="rds_backup_policy_"+$todaydate+".csv"
...
foreach ($regionid in $region)
{
	 ...
	 $rdsdbinstanceStorageType=($rdsinstancename_array.items.dbinstance.DBInstanceStorageType -split "`n")
        ...
        if (!$rdsdbinstanceid) 
           ...
           {
           $hh=/usr/local/bin/aliyun rds DescribeBackupPolicy --DBInstanceId $i
           ...
           $g2=$gg|Select-Object @{name="aliyunAccountName";expression={$s}},@{name="ProjectName";expression={$projectName.$i}},@{name="Region";expression={$regionid}},@{name="ApplicationName";expression={$rdsInstanceInfo_applicationname.$i}},@{name="Engine";expression={$rdsInstanceInfo.$i}},@{name="StorageType";expression={$rdsInstanceInfo_StorageType.$i}},@{name="DBInstanceId";expression={$i}},@{name="BackupMethod";expression={$_.BackupMethod}},@{name="BackupRetentionPeriod";expression={$_.BackupRetentionPeriod}},@{name="EnableIncrementDataBackup";expression={$_.EnableIncrementDataBackup}},@{name="LogBackupRetentionPeriod";expression={$_.LogBackupRetentionPeriod}},@{name="BackupPeriod";expression={$_.PreferredBackupPeriod}},@{name="BackupTime";expression={$_.PreferredBackupTime}}
           $f1=$f1+$g2
           }
           }
     ...
     $polardbclusternameStorageType=($polardbclustername_array.items.DBCluster.DBClusterStorageType -split "`n")
        ...
        if (!$polardbclustername_dbclusterid)
           ...
           foreach ($i in $polardbInstanceInfo.keys)
             {
             $hh=/usr/local/bin/aliyun polardb DescribeBackupPolicy --DBClusterId $i
             $gg=$hh|convertfrom-json
             $g3=$gg|Select-Object @{name="AccountName";expression={$s}},@{name="ProjectName";expression={$projectName.$i}},@{name="Region";expression={$regionid}},@{name="ApplicationName";expression={$polardbInstanceInfo_applicationname.$i}},@{name="Engine";expression={$polardbInstanceInfo.$i}},@{name="StorageType";expression={$polardbInstanceInfo_StorageType.$i}},@{name="DBInstanceId";expression={$i}},@{name="BackupMethod";expression={"Snapshot"}},@{name="BackupRetentionPeriod";expression={$_.DataLevel1BackupRetentionPeriod}},@{name="EnableIncrementDataBackup";expression={"NA"}},@{name="LogBackupRetentionPeriod";expression={"NA"}},@{name="BackupPeriod";expression={$_.DataLevel1BackupPeriod}},@{name="BackupTime";expression={$_.DataLevel1BackupTime}}
     	     $f2=$f2+$g3
...
#$x | export-excel ./$outputexcelfilename -Append
...
$Subject = "/usr/local/bin/aliyun RDS/polardb backup policy"
