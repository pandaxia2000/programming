     ...
     $rdsdbinstanceDescription=($rdsinstancename_array.items.dbinstance.DBInstanceDescription -split "`n")
        ...
        if (!$rdsdbinstanceid) 
           ...
           if ($g2.SSLExpireTime)
		   {
             ...
             if ($leftdays -lt 100 -and $leftdays -gt -100)
                 {
				  $h1='aliyun1 rds1 ModifyDBInstanceSSL1 --ConnectionString1 '+$g2.ConnectionString+' --DBInstanceId1 '+$i
				  $f1=$f1+$h1
			     }
           } 
           }
        ...
        if (!$polardbclustername_dbclusterid)
           ...
           foreach ($i in $polardbInstanceInfo.keys)
             ...
             if ($g3.SSLExpireTime)
		     {
			   ...
			   if ($leftdays -lt 100 -and $leftdays -gt -100)
			     {
				  $h2='aliyun1 polardb1 ModifyDBClusterSSL1 --DBClusterId '+$i
				  $f2=$f2+$h2
			     }
             }
             }
