...
logging.basicConfig(filename='WQ00671P5_log.txt', level=logging.DEBUG,
...
def get_result(actions_filename,templete_filename):
     datalist1=[]
     ...
     if result1:
        ...
        for z,f in enumerate(columns):
             actions_filename.write(0,z,f)
     
        for i,j in enumerate(result1,start=1):
           for x in range(len(j)):
              actions_filename.write(i,x,str(j[x]))
...
def loginout(filename):
     ...
     if result6:
        columns = [column[0] for column in cursor.description]
        for z,f in enumerate(columns):
            filename.write(0,z,f)
     
        for i,j in enumerate(result6,start=1):
            for x in range(len(j)):
               filename.write(i,x,str(j[x]))
...
try:
   get_result('data_change_actions','data_change_actions_templete')
except:
...
try:
   get_result('Permission_actions','permission_actions_templete')
except:
   logging.debug(traceback.format_exc())
else:
   logging.debug(traceback.format_exc())
