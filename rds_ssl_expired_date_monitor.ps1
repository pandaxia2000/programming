        ...
        if (!$rdsdbinstanceid) 
           ...
           if ($g2.SSLExpireTime)
		   {
             ...
             if ($leftdays -lt 30 -and $leftdays -gt -100)
                 {
				  $h1="SSL certification of "+$g2.ApplicationName+" is going to be expired within "+$leftdays+" ,  project name: "+$g2.projectname+" ,region: "+$g2.region+" , engine: "+$g2.engine+" ; "
			      $f1=$f1+$h1
			     }
           } 
           }
        ...
        if (!$polardbclustername_dbclusterid)
           ...
           foreach ($i in $polardbInstanceInfo.keys)
             ...
             if ($g3.SSLExpireTime)
		     {
			   ...
			   if ($leftdays -lt 30 -and $leftdays -gt -100)
			     {
				  $h2="SSL certification of "+$g3.ApplicationName+" is going to be expired within "+$leftdays+" ,  project name: "+$g3.projectname+" ,region: "+$g3.region+" , engine: "+$g3.engine+" ; "
			      $f2=$f2+$h2
			     }
             }
             }
...
$SMTPPort = "587"
Send-MailMessage -From $From -to $To -Cc $Copy -Subject $Subject -Body $body.Replace(";","`n") -SmtpServer $SMTPServer -Port $SMTPPort
