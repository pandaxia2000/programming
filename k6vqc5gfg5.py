...
def connectdatabase(databasename):
     server = 'k6vqc5gfg5.database.chinacloudapi.cn'
     database = databasename
     username = 'ethicaladmin'
     password = 'Q12345%qq'
...
try:
   ...
   '''
   temptablesql='''
   select distinct action_id, session_server_principal_name,database_name into #all_actions
   from fn_get_audit_file('https://ca041audit.blob.core.chinacloudapi.cn/sqldbauditlogs/##########',default,default)
   where event_time>=convert(varchar(10),getdate()-30,120);
   
   select distinct action_id, session_server_principal_name,database_name into #all_actions_last_month
   from fn_get_audit_file('https://ca041audit.blob.core.chinacloudapi.cn/sqldbauditlogs/##########',default,default)
   ...
   joinedlist = userlist_sql1+joinedlist+userlist_sql2
   ...
   from sys.database_principals a left join #all_actions_distinct actions on a.name=actions.username
   ...
   a.type,a.create_date,a.modify_date 
   from #all_actions_distinct all_actions left join 
