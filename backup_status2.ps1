...
$polardbclustername_applicationname=($polardbclustername_array.Items.DBCluster.DBClusterDescription -split "`n")
   ...
   if (!$polardbclustername_dbclusterid)
      ...
      foreach ($i in $polardbInstanceInfo.keys)
        ...
        $gg=$hh|convertfrom-json
        $gg.items.backup|Select-Object @{name="AliyunAccountName";expression={$s}},@{name="ApplicationName";expression={$polardbInstanceInfo_applicationname.$i}},@{name="Engine";expression={$polardbInstanceInfo.$i}},@{name="DBInstanceId";expression={$i}},@{name="BackupType";expression={$_.BackupType}},@{name="BackupStartTime";expression={$_.BackupStartTime}},@{name="BackupEndTime";expression={$_.BackupEndTime}},@{name="BackupStatus";expression={$_.BackupStatus}},@{name="BackupMethod";expression={$_.BackupMethod}},@{name="BackupSetSize";expression={$_.BackupSetSize}} |Format-Table -AutoSize -Wrap | out-file ./rds_backup_logs.txt -Append -Width 1000
...
$Body = "$todayissue abnormality was found for today.
$totalissue abnormality was found in today's report for past 7 days."
